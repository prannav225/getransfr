...
class RTCService {
    ...
    private transferRequests = new Map<string, { files: File[], callbacks: FileProgress }>();
    ...
    private async handleMetadataMessage(peerId: string, files: any[]): Promise<void> {
        const result = await new Promise<{ accepted: boolean, handle?: any }>(resolve => {
            eventBus.emit(EVENTS.FILE_TRANSFER_REQUEST, {
                peerId,
                files,
                handleAccept: (handle?: any) => resolve({ accepted: true, handle }),
