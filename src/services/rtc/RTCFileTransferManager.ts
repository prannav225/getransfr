...
class RTCFileTransferManager {
    ...
    public initializeSender(peerId: string, files: File[], callbacks: FileProgress): void {
        ...
        console.log(`[RTC] Sender initialized for peer ${peerId}. Total batch: ${totalSize} bytes`);
    }

    public async initializeReceiver(peerId: string, files: any[], fileSystemHandle?: any): Promise<void> {
        const totalSize = files.reduce((acc, f) => acc + f.size, 0);
        this.totalSizes.set(peerId, totalSize);
        this.sentSizes.set(peerId, 0);
        ...
        if (fileSystemHandle) {
            try {
                if (files.length === 1 && fileSystemHandle.kind === 'file') {
                    this.fileStreams.set(peerId, await fileSystemHandle.createWritable());
        ...
        console.log(`[RTC] Receiver initialized for ${peerId}. Expecting ${files.length} files.`);
    }

    public async sendFile(peerId: string, file: File, dataChannel: RTCDataChannel): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            worker.onmessage = async (e: MessageEvent) => {
                ...
                if (type === 'chunk') {
                    // Backpressure Check
                    if (dataChannel.bufferedAmount > RTCFileTransferManager.BUFFER_THRESHOLD) {
                        await new Promise<void>(res => {
                            ...
                            poller = setInterval(() => {
                                if (dataChannel.bufferedAmount < RTCFileTransferManager.BUFFER_THRESHOLD / 2) {
                                    onLow();
                                }
                            }, 50);
                    ...
                    if (now - lastUiUpdate > 100 || offset >= file.size) {
                        ...
                        const progress = Math.min(100, Math.round((totalSent / batchTotal) * 100));
                        const speed = totalSent / ((now - (this.startTime.get(peerId) || now)) / 1000 || 0.001);
                        
                        this.callbacks.get(peerId)?.onProgress(progress);
                        eventBus.emit(EVENTS.TRANSFER_STATS_UPDATE, {
                            peerId, speed, progress, totalSize: batchTotal, sentSize: totalSent
                        });
                        lastUiUpdate = now;
                    }

                    if (offset < file.size) {
                        worker.postMessage({ type: 'read', file, offset, chunkSize: RTCFileTransferManager.CHUNK_SIZE });
                    } else {
                        worker.terminate();
    ...
    public async addChunk(peerId: string, data: ArrayBuffer): Promise<void> {
        ...
        queue.push(async () => {
            ...
            if (!stream && this.dirHandles.has(peerId)) {
                try {
                    const handle = await this.dirHandles.get(peerId)!.getFileHandle(currentFile.name, { create: true });
            ...
            const receivedNow = (this.sentSizes.get(peerId) || 0) + chunkData.byteLength;
            this.sentSizes.set(peerId, receivedNow);

            const progress = Math.min(100, Math.round((receivedNow / totalBatch) * 100));
            const now = Date.now();
            ...
            eventBus.emit(EVENTS.FILE_TRANSFER_PROGRESS, {
                peerId, progress, speed, receivedSize: receivedNow, totalSize: totalBatch
            });
        });
    }
        ...
        await queue.push(async () => {
            ...
            if (stream) {
                ...
                if (fileChunks && fileChunks.length > 0) {
                    ...
                    const blob = new Blob(fileChunks as any, { type: fileMeta?.type || 'application/octet-stream' });
